...
import Kernel, { Context, Next } from '@macchiatojs/kernel'
...
describe('cors.test.js', () => {
  ...
  describe('options.origin=*', () => {
    ...
    beforeEach(() => {
      app = new Kernel({ expressify: false })
      app.use(cors({ expressify: false, origins: '*' }))
      app.use((ctx: Context) => {
        ctx.response.body = { foo: 'bar' }
      })
    })
  ...
  describe('options.exposeHeaders', () => {
    ...
    it('should Access-Control-Expose-Headers: `content-length`', function(done) {
      app = new Kernel({ expressify: false })
      app.use(cors({ expressify: false, exposeHeaders: 'content-length' }))
      app.use((ctx: Context) => {
        ctx.response.body = { foo: 'bar' }
      })
  ...
  describe('options.maxAge', () => {
    ...
    beforeEach(() => {
      app = new Kernel({ expressify: false })
      app.use(cors({ expressify: false, maxAge: 3600 }))
      app.use((ctx: Context) => {
        ctx.response.body = { foo: 'bar' }
      })
    })
  ...
  describe('options.credentials', () => {
    ...
    beforeEach(() => {
      app = new Kernel({ expressify: false })
      app.use(cors({ expressify: false, credentials: true}))
      app.use((ctx: Context) => {
        ctx.response.body = { foo: 'bar' }
      })
    })
  ...
  describe('options.allowHeaders', () => {
    it('should work with allowHeaders is string', async () => {
      const app = new Kernel({ expressify: false })
      app.use(cors({ expressify: false, allowHeaders: 'X-PINGOTHER' }))
      app.use((ctx: Context) => {
        ctx.response.body = { foo: 'bar' }
      })
